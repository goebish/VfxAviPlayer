...
BYTE * JpegFile::JpegFileToRGB(char* fileName,
							   UINT *width,
							   UINT *height)

{
	...
	*/
	fclose(infile);
	//fclose(jpeg_FILE);
	VFXjpeg_opened = false; // goeb
	jpeg_filename=NULL; // goeb
...
BOOL JpegFile::GetJPGDimensions(char* fileName,
								UINT *width,
								UINT *height)
...
BOOL JpegFile::RGBToJpegFile(char* fileName, 
							BYTE *dataBuf,
							UINT widthPix,
							UINT height,
							BOOL color, 
