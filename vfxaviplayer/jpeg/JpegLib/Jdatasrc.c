...
#define INPUT_BUF_SIZE  4096*1000	/* goeb choose an efficiently fread'able size */
...
fill_input_buffer (j_decompress_ptr cinfo)
{
  ...
  if(!VFXjpeg_opened) // goeb (wtf the file is closed when we first come here :/ )
  {
	  if(!jpeg_filename)
		  return FALSE;
		src->infile = fopen( jpeg_filename ,"rb");
		VFXjpeg_opened = src->infile != NULL;
		if(!VFXjpeg_opened)
			return FALSE; // shit happens
		
  }
	// not cool, but because of this bug we've to load the entire file in 1 time ...
